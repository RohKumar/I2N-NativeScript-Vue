              ...
              <!--  <CardView class="card" margin="10" elevation="20" @tap="seeDetails(item)"> -->
                  <ListView ref="listview" separatorColor="transparent" for="item in cart" :key="index">
                    ...
                    <GridLayout class="card-layout" rows="50" columns="50, *,*,*">
                        <Image :src="item.itemImage" stretch="aspectFill" col="0" row="0" />
                        <Label :text="item.itemName" class="" row="0" col="1" />
                        <Label :text="item.price" class="" row="0" col="2" />
                    </GridLayout>
                     </v-template>
                    </ListView>
                <!-- </CardView> -->
                <StackLayout class="line" />
                <!-- <MDCardView class="card" margin="10" elevation="20"> -->
                    <GridLayout class="card-layout" rows="50" columns="50, *,*">
                        <Label text="Total:" class="" row="0" col="1" />
                        <Label text="10$" class="" row="0" col="2" />
                    </GridLayout>
                <!-- </MDCardView>  -->
            </StackLayout>
        </ScrollView>
        ...
        submitPayment() {
            ...
            if (isAndroid) {
                ...
                this.stripeObj.createToken(cardobj, (error, token) => {
                    if (!error) {
                        console.log(token)
                        console.log("Connection with Server Successfull")
                        alert("Processing Payment")
                        this.submitStripePayment(token.id, this.buyer.email, this.tot).then(() => {
                            alert("Payment sent").then(() => {
                                that.$navigateBack()
                            })
                        }).catch(err => {
